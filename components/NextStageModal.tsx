...
interface Props {
  ...
  matches: Match[];
  onConfirm: (nextFormat: EliminationType, qualifiedIds: string[], groupAssignments?: Record<string, string>) => void;
  onClose: () => void;
...
export const NextStageModal: React.FC<Props> = ({ participants, matches, onConfirm, onClose, qualifiesByGroup, currentStage, currentStageNumber }) => {
  ...
  const canConfirm = selectedIds.length >= 2 && isValidCount;

  const handleConfirm = () => {
    if (!canConfirm) return;
    onConfirm(nextFormat, selectedIds, nextFormat === EliminationType.ROUND_ROBIN_2 ? groupAssignments : undefined);
  };

  // Order by wins, points in favor, points against (already done by calculateStandings sortFn)
  const sortedStandings = [...standings].sort((a, b) => (a.globalRank || 999) - (b.globalRank || 999));
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/80 backdrop-blur-sm p-4">
      <div className="bg-slate-800 border border-slate-700 w-full max-w-2xl rounded-xl shadow-2xl flex flex-col max-h-[90vh] animate-in fade-in zoom-in duration-200">
        ...
        <div className="p-6 overflow-y-auto flex-1 space-y-6">
          ...
          <div>
            <label className="block text-sm font-medium text-slate-400 mb-3">Select Qualified Participants</label>
              ...
              <table className="w-full text-sm text-left">
                <thead className="bg-slate-900 text-slate-500 uppercase text-xs font-bold">
                  <tr>
                    ...
                    <th className="px-4 py-2 text-center">Prev Group</th>
            ...
            <p className="mt-2 text-xs text-slate-500">
              ...
              {nextFormat === EliminationType.SINGLE_ELIMINATION && selectedIds.length % 2 !== 0 && (
